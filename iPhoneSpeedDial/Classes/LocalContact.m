...
- (id)initWithPrimaryKey:(NSInteger)pk database:(sqlite3 *)db {
    if (self = [super init]) {
		...
		if (init_statement == nil) {
            const char *sql = "SELECT combo, phone FROM contact WHERE abrecordid=?";
		...
		ABRecordRef person = ABAddressBookGetPersonWithRecordID(addressBook, primaryKey);
...
- (void)deleteFromDatabase {
    if (delete_statement == nil) {
        const char *sql = "DELETE FROM contact WHERE abrecordid=?";
...
- (void)dehydrate {
    if (dirty) {
        if (dehydrate_statement == nil) {
            const char *sql = "UPDATE contact SET combo=?,phone=? WHERE abrecordid=?";
